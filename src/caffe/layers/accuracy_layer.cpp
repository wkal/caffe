...
void AccuracyLayer<Dtype>::Reshape(
  ...
  (*top)[0]->Reshape(n_labels, n_labels, 1, 1);
...
void AccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  vector<vector<Dtype> > class_confusion(n_labels,class_accuracy);
  ...
  for (int i = 0; i < num; ++i) {
    ...
    for (int k = 0; k < top_k_; k++) {
    	class_confusion[static_cast<int>(bottom_label[i])][bottom_data_vector[k].second]++;
    /*
  ...
  Dtype* vec = (*top)[0]->mutable_cpu_data();
  for (int i=0; i < n_labels; ++i){

	  for (int j=0; j < n_labels; ++j)
	  {

		  const int idx = i*n_labels + j;
	  //LOG(INFO) << num_per_class[i];
		  vec[idx] = class_confusion[i][j];
	  }
